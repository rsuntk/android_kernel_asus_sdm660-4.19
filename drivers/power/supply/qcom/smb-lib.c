...
{
	cancel_delayed_work(&chg->asus_min_monitor_work);
	queue_delayed_work(system_power_efficient_wq, &chg->asus_min_monitor_work, msecs_to_jiffies(time));
}
...
void jeita_rule(void)
{
	...
	int rc, bat_volt, bat_temp, bat_health, bat_capacity;
	u8 charging_enable, FV_CFG_reg_value, FCC_reg_value, FV_reg, ICL_reg;
	...
	if (rc < 0)
		pr_warn("%s: Failed to set JEITA_EN_CFG_REG\n", __func__);

	rc = smblib_read(smbchg_dev, FLOAT_VOLTAGE_CFG_REG, &FV_reg);
	if (rc < 0)
		pr_warn("%s: Couldn't read FLOAT_VOLTAGE_CFG_REG\n", __func__);

	rc = smblib_read(smbchg_dev, USBIN_CURRENT_LIMIT_CFG_REG, &ICL_reg);
	if (rc < 0)
		pr_warn("%s: Couldn't read USBIN_CURRENT_LIMIT_CFG_REG\n", __func__);
	...
	case JEITA_STATE_RANGE_0_to_100:
		...
		if (rc < 0) {
			pr_warn("%s: SW_recharge failed rc = %d\n", __func__, rc);
		}
		break;
...
void asus_min_monitor_work(struct work_struct *work)
{
	...
	if (asus_get_prop_usb_present(smbchg_dev)) {
		...
		queue_delayed_work(system_power_efficient_wq, &smbchg_dev->asus_min_monitor_work, msecs_to_jiffies(ASUS_MONITOR_CYCLE));
...
void asus_chg_flow_work(struct work_struct *work)
{
	...
	case FLOAT_CHARGER_BIT:
		...
		if (rc < 0)
			pr_warn("%s: Couldn't read fast_CURRENT_LIMIT_CFG_REG\n", __func__);
			set_icl = ICL_500mA;
		rc = smblib_masked_write(smbchg_dev, USBIN_CURRENT_LIMIT_CFG_REG,
			USBIN_CURRENT_LIMIT_MASK, set_icl);
		if (rc < 0)
			pr_warn("%s: Failed to set USBIN_CURRENT_LIMIT\n", __func__);
	...
	case CDP_CHARGER_BIT:
		...
		break;
	case OCP_CHARGER_BIT:
		pr_debug("asus_chg_flow_work enter OCP_CHARGER_BIT");
	...
	case DCP_CHARGER_BIT:
		// Get current ICL
		pr_debug("%s Enter DCP_CHARGER_BIT\n", __func__);
		rc = smblib_read(smbchg_dev, USBIN_CURRENT_LIMIT_CFG_REG, &USBIN_1_cc);   //reg=1370
		if (rc < 0)
			pr_warn("%s: Couldn't read fast_CURRENT_LIMIT_CFG_REG\n", __func__);
		...
		queue_delayed_work(system_power_efficient_wq, &smbchg_dev->asus_adapter_adc_work, msecs_to_jiffies(15000));
		break;
	default:
		asus_smblib_relax(smbchg_dev);
		break;
...
{
	int rc;
	u8 usb_max_current, USBIN_CURRENT_LIMIT_reg;
	...
	rc = smblib_read(smbchg_dev, USBIN_CURRENT_LIMIT_CFG_REG, &USBIN_CURRENT_LIMIT_reg);
	if (rc < 0)
		pr_warn("%s: Failed to set USBIN_OPTIONS_1_CFG_REG\n", __func__);
...
static void smblib_micro_usb_plugin(struct smb_charger *chg, bool vbus_rising)
{
	if (vbus_rising) {
		...
		if (!asus_flow_processing) {
			pr_debug("%s: Charger plugged in \n", __func__);
			asus_flow_processing = 1;
			asus_insertion_initial_settings(smbchg_dev);
			asus_smblib_stay_awake(smbchg_dev);
			queue_delayed_work(system_power_efficient_wq, &smbchg_dev->asus_chg_flow_work, msecs_to_jiffies(12000));
		}
#endif
