...
uint8_t gest_event[GEST_SUP_NUM] = {
	...
	/* Huaqin add for ZQL1820-701 by zhangxiude at 2018/9/19 end */
	0x81, 0x1D, 0x2D, 0x3D, 0x1F, 0x2F, 0x51, 0x52, 0x53, 0x54
};

/*gest_event mapping to gest_key_def*/
uint16_t gest_key_def[GEST_SUP_NUM] = {
	/* Huaqin add for ZQL1820-701 by zhangxiude at 2018/9/19 start */
	GESTURE_EVENT_DOUBLE_CLICK /*KEY_POWER*/, GESTURE_EVENT_SWIPE_UP, 252,
	253, 254, GESTURE_EVENT_C, GESTURE_EVENT_Z, 257, 258, GESTURE_EVENT_S,
	GESTURE_EVENT_V, GESTURE_EVENT_W, GESTURE_EVENT_E, 263, 264, 265,
	/* Huaqin add for ZQL1820-701 by zhangxiude at 2018/9/19 end */
		...
		if (enable) {
			if (atomic_inc_return(&(data->lcm_lab_power)) == 1) {
				...
				if (rc) {
					...
					E("Regulator lcm_lab enable failed rc=%d",
					  rc);
				}
			} else {
				atomic_dec(&(data->lcm_lab_power));
			...
			if (atomic_inc_return(&(data->lcm_ibb_power)) == 1) {
				...
				if (rc) {
					...
					E("Regulator lcm_ibb enable failed rc=%d",
					  rc);
				}
			} else {
				atomic_dec(&(data->lcm_ibb_power));
			...
			if (atomic_dec_return(&(data->lcm_lab_power)) == 0) {
				...
				if (rc) {
					...
					E("Regulator lcm_lab disable failed rc=%d",
					  rc);
				}
			} else {
				atomic_inc(&(data->lcm_lab_power));
			...
			if (atomic_dec_return(&(data->lcm_ibb_power)) == 0) {
				...
				if (rc) {
					...
					E("Regulator lcm_ibb disable failed rc=%d",
					  rc);
				}
			} else {
				atomic_inc(&(data->lcm_ibb_power));
...
const struct seq_operations himax_info_seq_ops = { .start = himax_start,
						   .next = himax_next,
						   .stop = himax_stop,
...
{
	himax_info_proc_entry = proc_create(HIMAX_INFO_PROC_FILE, 0777, NULL,
					    &himax_info_proc_fops);
...
static ssize_t himax_test_show(struct device *dev,
...
static ssize_t himax_test_store(struct device *dev,
...
int himax_test_node_init(struct platform_device *tpinfo_device)
{
	...
	if (0 != err) {
		printk("[himax-ito] %s() - ERROR: sysfs_create_group() failed.",
		       __func__);
		sysfs_remove_group(&tpinfo_device->dev.kobj,
				   &ito_test_attribute_group);
		return -EIO;
	} else {
		printk("[himax-ito] %s() - sysfs_create_group() succeeded.",
		       __func__);
	}
	return err;
}
...
static int himax_palm_detect(uint8_t *buf)
{
	...
	I(" %s HX_PALM_REPORT_loopi=%d,base=%x,X=%x,Y=%x,W=%x \n", __func__,
	  loop_i, base, x, y, w);
	if ((!atomic_read(&ts->suspend_mode)) && (x == 0xFA5A) &&
	    (y == 0xFA5A) && (w == 0x00))
		return PALM_REPORT;
	else
		return NOT_REPORT;
...
int himax_common_proc_init(void)
{
	...
	himax_proc_GESTURE_file =
		proc_create(HIMAX_PROC_GESTURE_FILE,
...
static int himax_ts_work_status(struct himax_ts_data *ts)
{
	...
	if (atomic_read(&ts->suspend_mode) &&
	    !(((gesture_mode & 0x100) == 0) || ((gesture_mode & 0x0FF) == 0)) &&
				...
				int ts_path, int ts_status)
{
	...
	if ((hx_esd_event == length || hx_zero_event == length) &&
	    ...
	    (hx_touch_data->diag_cmd == 0) && (ts->in_self_test == 0)) {
		shaking_ret = g_core_fp.fp_ic_esd_recovery(
	...
	if (tp_key_index != 0x00) {
		...
		if (tp_key_index == 0x01) {
			...
			if (ts->pdata->virtual_key) {
				...
				input_report_key(ts->input_dev, KEY_APPSELECT,
						 1);
			}
		}
		input_sync(ts->input_dev);
			      ...
			      int ts_status)
{
	...
	if (ts_status == HX_REPORT_DATA || ts_status == HX_TS_NORMAL_END) {
		ts_status =
			himax_distribute_touch_data(buf, ts_path, ts_status);
...
/* Huaqin add for ZQL1820-701 by zhangxiude at 2018/9/19 end */
	...
	ts->ts_SMWP_wake_lock =
		wakeup_source_register(NULL, "HIMAX_common_NAME");
#endif
#ifdef HX_HIGH_SENSE
	ts->HSEN_enable = 0;
#endif
