...
static void hx83112a_read_ic_ver(void)
{
	...
	if ((tmp_data[0] == 0x44) || (tmp_data[0] == 0x77)) {
		...
		himax_in_parse_assign_cmd(hx83112_e8_ic_fw, tmp_addr,
					  sizeof(tmp_addr));
		g_core_fp.fp_register_read(tmp_addr, DATA_LEN_4, tmp_data, 0);
		I("E8=%X\n", tmp_data[0]);
		ic_data->vendor_ic_ver = tmp_data[0];
...
static void hx83112_dynamic_fw_name(uint8_t ic_name)
{
	...
	} else if ((ic_data->vendor_ic_ver == 0x03) ||
		   (ic_data->vendor_ic_ver == 0x13) ||
		   (ic_data->vendor_ic_ver == 0x23)) { /* Cut 3 */
		memcpy(firmware_name, "Himax_firmware_3.bin",
		       sizeof(char) * strlen("Himax_firmware_3.bin"));
	} else if ((ic_data->vendor_ic_ver == 0x04) ||
		   (ic_data->vendor_ic_ver == 0x14)) { /* Cut 4 */
		memcpy(firmware_name, "Himax_firmware_4.bin",
		       sizeof(char) * strlen("Himax_firmware_4.bin"));
	} else { /* Original */
		memcpy(firmware_name, "Himax_firmware.bin",
		       sizeof(char) * strlen("Himax_firmware.bin"));
	}

	i_CTPM_firmware_name = kzalloc(
		(sizeof(char) * (strlen(firmware_name) + 1)), GFP_KERNEL);
	if (i_CTPM_firmware_name != NULL) {
		memcpy(i_CTPM_firmware_name, firmware_name,
...
static int himax_hx83112_probe(void)
{
	...
	if (g_core_chip_dt == NULL) {
		g_core_chip_dt = kzalloc(sizeof(struct himax_chip_detect) *
						 HX_DRIVER_MAX_IC_NUM,
					 GFP_KERNEL);
